...
{
    public class EfUserRelationshipDal : EfEntityRepositoryBase<UserRelationship, ContextDb>, IUserRelationshipDal
    {
        ...
        public List<UserRelationshipDto> GetListDto(int adminUserId)
        {
            using (var context = new ContextDb())
            {
                var result = from userReletionShip in context.UserRelationships.Where(p => p.AdminUserId == adminUserId)
